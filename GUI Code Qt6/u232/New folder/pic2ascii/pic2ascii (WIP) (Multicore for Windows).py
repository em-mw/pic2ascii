    ...
    if not file_path_list:
        ...
        for thing in range(properseq):
            ...
            if len(file_path_list) == int(properseq):
                # truncate_utf8_chars(str(os.getcwd()) + str(dirslash) + str(int(thing + 1)) + str(dirslash) + 'tmp' + '.tmp', 1)
                # truncate_utf8_chars(str(os.getcwd()) + str(dirslash) + str(int(thing + 1)) + str(dirslash) + 'tmp' + '.tmp', 1)
                tmp.write(']')
            else:
                tmp.write(', ')
            ...
            tmp.close()
        if str(int(len(file_path_list)) / int(properseq)).find('.') != int(-1):
            if str(int(len(file_path_list)) / int(properseq))[int(2)].find('0') != int(0) and len(str(int(len(file_path_list)) / int(properseq))) <= 3 and os.path.exists(os.getcwd() + str(properseq) + 'tmp.tmp'):
